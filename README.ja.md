...
本ライブラリの中核機能は、ペアリング済みのブラウザセッションへ届いた署名リクエストを、本物の `SSH_AUTH_SOCK` Unix ドメインソケット（`agentServer.startUnixSocket()`）へ中継することです。このソケットを利用する `ssh` / `git` / `rsync` / `scp` プロセスは、サーバー上にもディスク上にも一切存在せず、ブラウザタブの中だけに存在する鍵を使って認証できます。

サーバー側の動かし方は2通りあり、そのどちらとも組み合わせられるウィジェットが1つあります。

- **`bssh-agent` CLI** — `ssh-agent` 形式のスタンドアロンデーモンです。ホストアプリは不要で、`SSH_AUTH_SOCK` が直接シェルに渡されます。詳しくは [スタンドアロン: `bssh-agent` CLI](#スタンドアロン-bssh-agent-cli) を参照してください。
- **`AgentServer` をアプリに組み込む** — すでに動かしている HTTP(S) サーバーにアタッチし、アプリが行う `child_process.spawn()` の呼び出しに `agentServer.env()` を展開します。詳しくは [組み込み: 自分の HTTP サーバーにアタッチする](#組み込み-自分の-http-サーバーにアタッチする) を参照してください。
- **`<bssh-agent-pairing>` ウィジェット**（`bssh-agent/widget`） — 下記のブラウザ側プリミティブをラップしたドロップイン Web Component です。ホストページ側で鍵入力フォームや署名確認ダイアログを自前で書く必要がなく、`<script>` タグとカスタムエレメントを置くだけで済みます。上記どちらの方式とも組み合わせられます。詳しくは [ブラウザ側, ドロップインウィジェットを使う](#ブラウザ側-ドロップインウィジェットを使う) を参照してください。
...
### スタンドアロン: `bssh-agent` CLI

`ssh-agent` 形式の、すぐに使えるデーモンです — ホストアプリもコード変更も不要です。人間が自分のシェルへ直接 `ssh` / `git` / `rsync` と入力するケースや、まずライブラリを試してみたいだけのケースに最適です。

```sh
eval "$(bssh-agent)"
```

これはバックグラウンドデーモンを起動し、ペアリング URL を stderr に出力し(`--no-browser` 指定時やリモート/SSH セッションが検出された場合を除き、既定のブラウザで開こうとします)、`SSH_AUTH_SOCK` / `SSH_AGENT_PID` を現在のシェルへ `eval` します。下記のウィジェットを使ってペアリングを済ませれば、それ以降そのシェルで実行する `ssh` / `git` / `rsync` は、ホストアプリ側のコード変更なしに自動的に `SSH_AUTH_SOCK` を利用できます。ペアリングが完了する前にコマンドが実行された場合は、単純に認証が失敗するだけで、ロック中の GUI キーチェーンエージェントに対する動作と同様、再試行が可能です。

```sh
bssh-agent -k    # デーモンを停止し、環境変数を unset します: eval "$(bssh-agent -k)"
```

フラグの完全な一覧(`-D` / `--foreground`、`--name`、`--force`、`--port`、`--host`、`--runtime-dir` など)は [APIリファレンス](./docs/REFERENCE.ja.md#bssh-agent-cli) を参照してください。
...
### 組み込み: 自分の HTTP サーバーにアタッチする

すでに構築しているアプリに `AgentServer` を組み込みます: その既存の HTTP(S) サーバーにアタッチし、Unix ソケットトランスポートを起動します。
...
```html
<script type="module" src="https://unpkg.com/bssh-agent@1.0.0/dist/widget/index.js"></script>
...
既定では、すべての署名リクエストがプロンプトなしで自動的に承認されます — これは本物の `ssh-agent` 自体の既定動作と同じです(`ssh -A` も署名のたびに確認を求めたりはしません)。代わりに署名ごとに組み込みの承認/拒否プロンプトを表示したい場合は `require-confirm="true"` を設定してください(本物の `ssh-agent` のオプションの `-c`/confirmモードと同じトレードオフです — [セキュリティに関する注意](#セキュリティに関する注意)を参照)。または `confirmSign` プロパティを設定して、組み込みのものの代わりに独自の UI を指定することもできます。

属性・プロパティ・イベントの完全な一覧は [APIリファレンス](./docs/REFERENCE.ja.md#bssh-agentwidget) を参照してください。

## Dockerで試す

Node をローカルにインストールせずにスタンドアロン CLI を試せる、自己完結した方法です。
...
最後のコマンドは `Pairing URL: http://127.0.0.1:8787/#token=...` という行を出力します — これをコンテナの中ではなく、自分のデスクトップのブラウザで開いてください。そのブラウザのファイル選択は、あなた自身のマシンのディスクから直接秘密鍵を読み込みます。コンテナ側は鍵を一切見ることがなく、その後 WebSocket を通過するのは公開鍵と署名だけです([仕組み](#仕組み)を参照)。最初に試すだけなら使い捨てのキーペアで十分です。ペアリング後は、署名がプロンプトなしで行われます(既定の動作です — トレードオフについては[セキュリティに関する注意](#セキュリティに関する注意)を、署名ごとに承認したい場合は `--require-confirm` を参照してください)。

ペアリングが済んだら、コンテナの中から実際の `ssh` コマンドを実行して試せます。
...
`git@github.com` の部分は自分のリモートホストに置き換えてください — ここでは追加のサーバー設定が不要という理由だけでこれを使っています。コンテナは Docker 自身のポート公開機能から到達できるよう、`--host` によってペアリングページを `0.0.0.0` にバインドしています。コンテナの外では、CLI の既定値はループバックのみのままです — [セキュリティに関する注意](#セキュリティに関する注意)を参照してください。
...
[組み込みの使い方例](#組み込み-自分の-http-サーバーにアタッチする)は、最も単純で安全なケースを前提としています。すなわち、ペアリングリンクを発行するプロセス自体が、ユーザーが既に自分のブラウザで見ているウェブアプリそのものであるケースです。この場合、`createPairingLink()` が返す URL は、そのページ上にリンクやボタンとして描画するだけでよく、他の配信経路は一切不要です。

この前提が成り立たないのは、`createPairingLink()` を呼び出す側にリンクを描画するブラウザが存在しない場合です。代表例が `bssh-agent` CLI（[スタンドアロン](#スタンドアロン-bssh-agent-cli) を参照）で、SSH 接続したリモート/ヘッドレスなマシン上で動いている可能性があります。この場合、URL を*別の*デバイスのブラウザへ届ける必要があり、よく使われる方法は、ユーザーにコピー＆ペーストしてもらうために印刷する方法と、スマートフォンで読み取れるよう QR コードとして描画する方法の2つです。
...
- **既定では、すべての署名リクエストがプロンプトなしで自動的に承認されます。** これは本物の `ssh-agent` 自体の既定動作と同じです(`ssh -A` も署名のたびに確認を求めたりはしません)。これが持つトレードオフは実際のエージェントフォワーディングと同じです — 中継しているサーバーが乗っ取られた場合、ブラウザタブが接続されている間、攻撃者はペアリング済みの鍵であなたとして認証できてしまいます(なお ssh-agent プロトコルの仕様上、どのリモートホストへの認証かはエージェント側からも分からず、鍵のフィンガープリントしか判別できないため、リクエストの内容だけから見分ける方法もありません)。ウィジェットの `require-confirm="true"`、あるいはCLIの `--require-confirm` を設定すると、署名のたびに承認/拒否プロンプトが表示されるようになります(本物の `ssh-agent` のオプションの `-c`/confirmモードと同じ緩和策です)。または `confirmSign` を直接指定して独自のUIを使うこともできます。
